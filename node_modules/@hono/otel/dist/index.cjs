...
__export(index_exports, {
  httpInstrumentationMiddleware: () => httpInstrumentationMiddleware
});
...
var getMeter = (config) => (config.meterProvider ?? import_api.metrics.getMeterProvider()).getMeter(INSTRUMENTATION_SCOPE.name);
var createRequestDurationTracker = (config) => {
  const histogram = getMeter(config).createHistogram(import_incubating.METRIC_HTTP_SERVER_REQUEST_DURATION, {
...
var createActiveRequestsTracker = (config) => {
  const counter = getMeter(config).createUpDownCounter(import_incubating.METRIC_HTTP_SERVER_ACTIVE_REQUESTS, {
...
var resolveTracer = (config) => {
  ...
  const provider = config.tracerProvider ?? import_api2.trace.getTracerProvider();
...
var httpInstrumentationMiddleware = (userConfig = {
  ...
  const spanName = (c) => config.spanNameFactory?.(c) ?? `${c.req.method} ${(0, import_route.routePath)(c)}`;
  ...
  return (0, import_factory.createMiddleware)(async (c, next) => {
    const parent = import_api2.propagation.extract(import_api2.context.active(), c.req.header());
    const method = c.req.method;
    const stableAttrs = {
      [import_semantic_conventions.ATTR_HTTP_REQUEST_METHOD]: method,
    ...
    for (const [rawName, value] of Object.entries(reqHeaders)) {
      ...
      if (config.requestHeaderSet.has(name)) {
        deferredRequestHeaderAttributes[(0, import_semantic_conventions.ATTR_HTTP_REQUEST_HEADER)(name)] = value;
      }
    }
    const finalize = (span, error) => {
      try {
        ...
        if (span) {
          ...
          for (const [name, value] of c.res.headers.entries()) {
            ...
            if (captureResp.has(lower)) {
              span.setAttribute((0, import_semantic_conventions.ATTR_HTTP_RESPONSE_HEADER)(lower), value);
            }
          }
          span.setAttribute(import_semantic_conventions.ATTR_HTTP_RESPONSE_STATUS_CODE, status);
          if (status >= 500) {
        ...
        span?.setAttribute(import_semantic_conventions.ATTR_HTTP_ROUTE, (0, import_route.routePath)(c));
        span?.updateName(spanName(c));
        requestDuration.record(performance.now() - monotonicStartTime, {
          ...stableAttrs,
          [import_semantic_conventions.ATTR_HTTP_ROUTE]: (0, import_route.routePath)(c),
          [import_semantic_conventions.ATTR_HTTP_RESPONSE_STATUS_CODE]: c.res.status
        });
      }
    };
    if (!tracer) {
    ...
    return tracer.startActiveSpan(
      spanName(c),
      {
        ...
        attributes: {
          ...stableAttrs,
          [import_semantic_conventions.ATTR_URL_FULL]: c.req.url,
          [import_semantic_conventions.ATTR_HTTP_ROUTE]: (0, import_route.routePath)(c)
        }
      },
      parent,
